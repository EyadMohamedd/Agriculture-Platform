...
export default function FarmValidationRanges() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([
      farmsApi.getValidationRanges(farmId),
      farmsApi.getAll({ pageSize: 100 })
    ]).then(([rangesRes, farmsRes]) => {
      setRanges(rangesRes.data.data || []);
      const farm = (farmsRes.data.data.items || []).find((f) => f.id === farmId);
  ...
  const handleSaved = () => { setDialog({ open: false, editing: null }); load(); };

  const handleDelete = async () => {
    if (!deleteDialog.range) return;
    setDeleting(true);
    try {
      await farmsApi.deleteValidationRange(farmId, deleteDialog.range.id);
    ...
    <Stack spacing={3}>
      <Stack direction="row" alignItems="center" spacing={2}>
        ...
        <Stack>
          ...
          {farmName && <Typography variant="body2" color="text.secondary">{farmName}</Typography>}
        </Stack>
      </Stack>
      ...
      <MainCard
        title="Farm-Specific Thresholds"
        secondary={
          !isAdmin && (
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, range: null })}>
        ...
        <DialogContent>
          <Typography>
            Delete the {sensorLabel(deleteDialog.range?.sensorType)} range? The system default will apply instead.
          </Typography>
        </DialogContent>
        <DialogActions>
